 ...
 public:
      ...
      threads_(numThreads)
  {
    for (int i = 0; i < numThreads; ++i)
    {
      ...
      threads_.push_back(new jmuduo::Thread(
            boost::bind(&Test::threadFunc, this), jmuduo::string(name)));
    }
  ...
  void run(int times)
  {
    ...
    for (int i = 0; i < times; ++i)
    {
      ...
      queue_.put(buf);
      printf("tid=%d, put data = %s, size = %zd\n", jmuduo::CurrentThread::tid(), buf, queue_.size());
    }
  }
 ...
 private:
  ...
  void threadFunc()
  {
    ...
    while (running)
    {
      ...
      printf("tid=%d, get data = %s, size = %zd\n", jmuduo::CurrentThread::tid(), d.c_str(), queue_.size());
      running = (d != "stop");
    }
  ...
  jmuduo::BoundedBlockingQueue<std::string> queue_;
